...
//      COMPLETED (7) Generate a weather summary with the date, description, high and low
        String weatherForThisDay = summarize();
//      COMPLETED (8) Display the summary that you created above
    ...
    private String summarize() {
        int weatherIdColumnIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID);
        int dateColumnIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_DATE);
        int maxTempColumnIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP);
        int minTempColumnIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP);

        return SunshineDateUtils.getFriendlyDateString(mContext, mCursor.getLong(dateColumnIndex), false) +
                SunshineWeatherUtils.getLargeArtResourceIdForWeatherCondition(mCursor.getInt(weatherIdColumnIndex)) +
                SunshineWeatherUtils.formatHighLows(mContext, mCursor.getDouble(maxTempColumnIndex),
                        mCursor.getDouble(minTempColumnIndex));
    }
