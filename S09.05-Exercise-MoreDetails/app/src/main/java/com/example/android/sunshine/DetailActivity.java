...
//  COMPLETED (19) Create constant int values representing each column name's position above
    ...
    private static final int INDEX_COLUMN_WIND_DEGREES = 6;
...
//      COMPLETED (13) Find each of the TextViews by ID
        ...
        high = (TextView) findViewById(R.id.tv_high_temp);
        low = (TextView) findViewById(R.id.tv_low_temp);
...
//      COMPLETED (26) Display a readable data string
        date.setText(SunshineDateUtils.getFriendlyDateString(this, data.getLong(INDEX_COLUMN_DATE), false));
//      COMPLETED (27) Display the weather description (using SunshineWeatherUtils)
        description.setText(SunshineWeatherUtils.getStringForWeatherCondition(this, data.getInt(INDEX_COLUMN_WEATHER_ID)));
//      COMPLETED (28) Display the high temperature
        high.setText(SunshineWeatherUtils.formatTemperature(this, data.getDouble(INDEX_COLUMN_MAX_TEMP)));
//      COMPLETED (29) Display the low temperature
        low.setText(SunshineWeatherUtils.formatTemperature(this, data.getDouble(INDEX_COLUMN_MIN_TEMP)));
//      COMPLETED (30) Display the humidity
        humidity.setText(String.valueOf(getString(R.string.format_humidity, data.getFloat(INDEX_COLUMN_HUMIDITY))));
//      COMPLETED (31) Display the wind speed and direction
        wind.setText(SunshineWeatherUtils.getFormattedWind(this, data.getFloat(INDEX_COLUMN_WIND_SPEED), data.getFloat(INDEX_COLUMN_WIND_DEGREES)));
//      COMPLETED (32) Display the pressure
        pressure.setText(getString(R.string.format_pressure, data.getFloat(INDEX_COLUMN_PRESSURE)));
//      COMPLETED (33) Store a forecast summary in mForecastSummary

        mForecastSummary = String.format("%s - %s - %s/%s", date.getText().toString(),
                description.getText().toString(), high.getText().toString(), low.getText().toString());
